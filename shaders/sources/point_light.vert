...
void main() {
    ...
    vec3 position_world = ubo.light_position.xyz
            + LIGHT_RADIUS * fragOffset.x * camera_right_world
            + LIGHT_RADIUS * fragOffset.y * camera_up_world;

    gl_Position = ubo.projection_matrix * ubo.view_matrix * vec4(position_world, 1.0f);
}